shinyServer(function(input, output) {
    ...
    # rval_pal <- reactive({
    #     map_bins <-c(min(rval_traffic()$adjavgdly), -5, 0, 5, 10, 15, 20, 25, 30, max(rval_traffic()$adjavgdly)) 
    #     colors = c("#00b81c", "#fafa00", "#eb0000")
    #     
    #     colorBin(palette=colors, bins=map_bins)
    # })
    ...
    output$scatter_plot <- renderPlotly({
        ...
        p <- rval_traffic() %>%
            ggplot(aes(x = adjavgdly, y = pct_change, label = location, key = key)) + 
